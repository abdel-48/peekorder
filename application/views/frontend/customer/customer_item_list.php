			...
			<tbody>
				...
				<?php foreach ($order['item_list'] as $key => $row): ?>
					<?php if($row['is_package']==1): ?>
						...
						$grand_total = grand_total($total_price,$order['delivery_charge'],$order['discount'],$order['tax_fee'],$order['coupon_percent'],$order['tips'],$order['order_type'],shop($row['shop_id'])->tax_status);
						?>
					<?php endforeach; ?>
					<tr class="bolder" >
						<td colspan="3"></td >
