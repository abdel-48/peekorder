						...
						<table class="table table-bordered table-condensed table-striped" id="example1">
							...
							<?php foreach ($order['item_list'] as $key => $row): ?>
								...
								<?php 
									...
									$grand_total = number_format(grand_total($total_price,$order['delivery_charge'],$order['discount'],$order['tax_fee'],$order['coupon_percent'],$order['tips'],$order['order_type'],shop($order['shop_id'])->tax_status),2);
								?>
							<?php endforeach ?>
							<tr class="bolder" >
								<td colspan="3"></td >
