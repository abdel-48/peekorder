	...
	<div class="step_2">
				...
				<div class="row">
					<div class="col-md-6">
						<div class="dineInList">
							...
							<select name="table_no" class="form-control table_no" id="table_no">
								<option value=""><?= lang('select'); ?></option>
								<?php foreach ($dinein as $key => $dine): ?>
									<option value="<?= $dine['id'];?>" data-size="<?=  $dine['size'];?>" <?= !empty(auth('table_no')) && auth('table_no')==$dine['id']?"selected":"";?>><?=  $dine['name'];?> - <?= $dine['area_name'];?></option>
								<?php endforeach ?>
							</select>
						</div>
					</div>
		...
		<div class="dis_none hidePay-in-cash show_address">
			...
			<div class="changeInfo dis_none">
				<div class="row mt-10">
					<div class="form-group col-md-6">
						<label class="custom-checkbox"><b><input type="checkbox" name="is_change" class="is_change"  value="1"> I need Change</b></label>
