				...
				<div class="card">
					...
					<div class="oder_info-2">
						...
						<?php else: ?>
							...
							<label class="label default-light-active" data-toggle="tooltip" title="Total Price"> <?= lang('price'); ?> : <?= currency_position(grand_total($row['total_price'],$row['delivery_charge'],$row['discount'],$row['tax_fee'],$row['coupon_percent'],$row['tips'],$row['order_type'],restaurant()->tax_status),restaurant()->id);?></label>
							<div class="mt-2">
								<label class="label bg-primary-soft" data-toggle="tooltip" title="Order Time"> <?= lang('order'); ?> Time: <?= full_time(html_escape($row['created_at']),restaurant()->id);?></label>
							</div>
						<?php endif;?>
