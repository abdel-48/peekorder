<div class="row">
	<?php include APPPATH.'views/backend/common/inc/leftsidebar.php'; ?>
	   <?php $data = !empty($settings['table_qr_config'])?json_decode($settings['table_qr_config'],TRUE):''; ?>
		<div class="col-md-4">
			<form class="validForm" action="<?= base_url('admin/restaurant/add_table_qr/'.$shop_id) ?>" method="post" enctype= "multipart/form-data" autocomplete="off">
				...
				<div class="card">
                    ...
                    <div class="row">
                        <div class="form-group col-md-8">
                            <label></label>
                            <select name="qr_type" id="qr_type" class="form-control" required>
                                <option value="waiter" <?= isset($data['qr_type']) && $data['qr_type']=="waiter"?"selected":"";?>><?= lang('call_waiter')?></option>
                                <option value="table" <?= isset($data['qr_type'])&& $data['qr_type']=="table"?"selected":"";?>><?= lang('table-dine-in')?></option>
                            </select>
                        </div>
                    </div>
				...
				<div class="card">
					...
					<div class="card-body text-center p-0">
						<div class="content with-padding">
                            <?php 
                                if(isset($data['qr_type']) && $data['qr_type']=='table'):
                                    $url = base_url(restaurant()->username.'/?table='.$data['table_no']);
                                else:
                                    $url = base_url('profile/qr_waiter/'.restaurant()->username.'/?table='.$data['table_no']);
                                endif;
                             ?>
                            <div id="qr-code-wrapper" class="margin-bottom-20" data-url="<?= $url;?>">
                            	<img src="" alt="">
                            </div>
