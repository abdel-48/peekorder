...
<div class="row">
	...
	<div class="col-md-5">
		<div class="row">
			<form class="email_setting_form" action="<?= base_url('admin/restaurant/add_pickup_points/'.$shop_id) ?>" method="post" enctype= "multipart/form-data" autocomplete="off">
		...
		<div class="row">
			<form class="email_setting_form" action="<?= base_url('admin/restaurant/add_pickup_slots/'.$shop_id) ?>" method="post" enctype= "multipart/form-data" autocomplete="off">
				<input type="hidden" name="<?= $this->security->get_csrf_token_name(); ?>" value="<?= $this->security->get_csrf_hash(); ?>" autocomplete="off">
				<div class="col-md-12">
					<div class="card">
							...
							<div class="row p-15">
								<div class="table-responsive">
									<div class="pickup-slots">
										<?php $pickup_slots = json_decode($restaurant['pickup_time_slots']); ?>
										<?php $get_time = getTimeSlot('30','00:00','23:00') ?>
										<?php foreach ($get_time as $key2 => $slots): ?>

										<label class="time_slot success-light <?= isset($pickup_slots) && in_array($slots['slot_start_time'].' - '.$slots['slot_end_time'],$pickup_slots)==1?"active":"" ;?>">

											<input type="checkbox" class="timeChecked" name="time_slots[]" value="<?= $slots['slot_start_time'] ;?> - <?= $slots['slot_end_time'] ;?>" <?= isset($pickup_slots) && in_array($slots['slot_start_time'].' - '.$slots['slot_end_time'],$pickup_slots)==1?"checked":"" ;?>> <?= time_format($slots['slot_start_time'],restaurant()->id) ;?> - <?= time_format($slots['slot_end_time'],restaurant()->id) ;?>
										</label>
