 ...
 <div class="userMenu ">
 	<div class="container">
	 	<nav class="navbar navbar-expand-lg navbar-light bg-light fixed-top">
	 		<div class="container">
				  <a class="navbar-brand" href="<?= base_url($slug) ;?>"><img src="<?=  !empty(restaurant($id)->thumb)? base_url(restaurant($id)->thumb):base_url("assets/frontend/images/logo-example.png") ?>" alt="" class="shopLogo">	</a>
				  <ul class="smDropdown">
					    	...
					    	<?php if(is_feature($id,'welcome')==1 && is_active($id,'welcome')): ?>
						  	...
						  	<?php endif;?>
						  		...
						  		<div class="dropdownArea" style="display: none;">
						  			<ul>
								  		<li class="nav-item allow-sm"><a class="nav-link" href="<?= base_url('track-order/'.$slug) ;?>"><?= lang('track_order'); ?></a></li>

								  		<?php if(is_feature($id,'reservation')==1 && is_active($id,'reservation')): ?>
								  			<li  class="nav-item allow-sm" ><a class="nav-link" href="<?= base_url('reservation/'.$slug);?>"><?= get_features_name('reservation');?></a></li>
								  		<?php endif;?>

								  		<?php if(is_feature($id,'contacts')==1 && is_active($id,'contacts')): ?>
								  		<li  class="nav-item allow-sm" ><a class="nav-link" href="<?= base_url('contacts/'.$slug);?>"><?= get_features_name('contacts');?></a></li>
								  	<?php endif;?>

									  	<li  class="nav-item allow-sm" ><a class="nav-link" href="<?= base_url('about-us/'.$slug);?>"><?= lang('about_us'); ?></a></li>
 	...
 	<div class="UserMobileMenu">
 		<ul>
 				...
 				<li data-toggle="tooltip" title="Home" class="<?= isset($page_title) && $page_title=="Profile"?"active":"" ;?>">
 					<?php if(is_feature($id,'welcome')==1 && is_active($id,'welcome')): ?>
	 					<a href="<?= base_url($slug) ;?>"><i class="icofont-home"></i></a>
	 				<?php endif;?>
 				</li>
 			

 			
	 			<li data-toggle="tooltip" title="Menu" class="<?= isset($page_title) && $page_title=="Menus"?"active":"" ;?>">
	 				<?php if(is_feature($id,'menu')==1 && is_active($id,'menu')): ?>
	 					<a href="<?= base_url('menu/'.$slug) ;?>"><i class="icofont-culinary fz-36"></i></a>
	 				<?php endif;?>
	 			</li>
 			...
 			<li data-toggle="tooltip" title=""><a href="javascript:;" class="base"><i class="icofont-gears"></i></a></li>
	 			...
	 			<li data-toggle="tooltip" title="Packages" class="<?= isset($page_title) && $page_title=="Packages"?"active":"" ;?>">
	 				<?php if(is_feature($id,'packages')==1 && is_active($id,'packages')): ?>
	 					<a href="<?= base_url('packages/'.$slug) ;?>"><i class="icofont-gift"></i></a>
	 				<?php endif;?>
	 			</li>
	 

	 		
	 			<li data-toggle="tooltip" title="Specialties" class="<?= isset($page_title) && $page_title=="Specialties"?"active":"" ;?>">
	 				<?php if(is_feature($id,'specialities')==1 && is_active($id,'specialities')): ?>
	 					<a href="<?= base_url('specialties/'.$slug) ;?>"><i class="icofont-touch"></i></a>
	 				<?php endif;?>
	 			</li>
