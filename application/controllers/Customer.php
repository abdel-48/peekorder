...
class Customer extends MY_Controller {
	...
	public function registration()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$insert = $this->common_m->insert($data,'staff_list');
			if($insert):
				$s_array = array(
					'customer_id' => $insert,
					'customer_name' => $name,
				...
				$this->session->set_userdata($s_array);
					...
					$info = '
						<div class="flex flex-column">
							...
							<div class="customerInfoModal">
								<h4 class="pb-7 pt-5 fz-14"><i class="icofont-users-alt-4"></i> '.$row->name.'</h4>
								<p class="fz-14"><i class="icofont-ui-call"></i> '.$row->phone.'</p>
							</div>
						</div>
						<div class="customerEdit">
							<a href="#customereditModal" data-toggle="modal"><i class="fa fa-edit"></i></a>
...
public function update_customer()
{
	...
	if ($this->form_validation->run() == FALSE) {
		...
		if($_POST){
				...
				if(isset($_POST['is_update']) && $_POST['is_update']==1):
					$this->admin_m->update($update_data,$this->input->post('customer_id',TRUE),'staff_list');
				endif;
 ...
 public function profile(){
	...
	if ($this->form_validation->run() == FALSE) {
		...
		$check_email = $this->admin_m->check_valid_staff_phone(trim($this->input->post('phone',true)));
...
public function recovery_password(){
	...
	if ($this->form_validation->run() == FALSE) {
		...
		$customer_data = $this->admin_m->single_select_by_id($id,'staff_list');
		...
		if(isset($question) && !empty($customer_data)):
			if($answer==$question->answer){
				...
				$this->common_m->update($data,$id,'staff_list');
